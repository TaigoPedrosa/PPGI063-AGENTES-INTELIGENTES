...
Taught specifically in the Claude Code course, these focus on a "Plan-Execute" loop rather than simple generation:
    - The "Plan-First" Workflow: Always ask the agent to create a plan in a [TASK.md](http://task.md/) or a markdown block before it writes any code. This allows for human review of the logic before implementation.
    - The [CLAUDE.md](http://claude.md/) File: Maintain a project-level configuration file that contains specific coding standards, build commands, and architectural notes that the agent should follow every time it enters the environment.
