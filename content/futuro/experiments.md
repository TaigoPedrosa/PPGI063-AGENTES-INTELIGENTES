...
> **Note on scope.** Unlike the *present* team's experiments (which the team ran themselves), the Futuro folder contains no original experiment. This page is a **digest of the experiments and benchmark results reported inside the papers** in `Futuro/pdfs/`. Every number below is quoted from the source paper, with the file named. Nothing here was generated or estimated by us. Where a paper doesn't report something, it is left out.
>
